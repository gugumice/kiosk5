...
def service_thread(th_ev: threading.Event, polling_int: float = 0.5, config: dict = config, queue_from_gui: Queue = None, queue_to_gui: Queue = None):
    ...
    while not th_ev.is_set():
        ...
        if not queue_from_gui.empty():
            ...
            ticket = kiosk_utils.Ticket(ticket_type=kiosk_utils.TicketPurpose.BCR,
                                        ticket_display_time=config['button_debounce_time_ms'])
            queue_to_gui.put(ticket)
            lang = msg
        bc_reader.next()
...
def bc_callback(*args) -> bool:
    ...
    logging.debug(f"Received barcode: {barcode}, lang: {lang}")
    ...
    if re.match(reg_ex, barcode):
        ...
        print(2)
        kiosk_utils.send_ticket(ticket_value=barcode,
                ticket_type=kiosk_utils.TicketPurpose.AOK,
                ticket_animate_once=True,
                queue_tx=queue_to_gui)
        print('-------------')
        # # time.sleep(1)
        # report_url = config['url'].format(config['host'],barcode,lang)
        ...
        #     logging.debug(report_url)
        #     kiosk_utils.send_ticket(ticket_type=kiosk_utils.TicketPurpose.NET,
        #             ticket_animate_once=True,
        #             queue_tx=queue_to_gui)
        #     r = kiosk_report.get_report_from_host(report_url, timeout = config['httpreq_timeout'])
        #     if r[0] == 200:
        #         kiosk_utils.speak_status(os.path.join(config['assets_loader'], 'start_print{}{}'.format(lang, '.wav')), background=True)           
        ...
        #         kiosk_utils.speak_status(os.path.join(config['assets_loader'], 'end_print{}{}'.format(lang, '.wav')), background=True) 
        #         kiosk_utils.send_ticket(ticket_type=kiosk_utils.TicketPurpose.AOK,
        #             ticket_animate_once=True,
        #             queue_tx=queue_to_gui)
        #     else:
        #         kiosk_utils.speak_status(os.path.join(config['assets_loader'], 'error_attn.wav'), background=False)
